...
import { DotsHorizontal, FilterLines, User_Circle, XClose } from "@/appIcons";
...
export default function Data({ openModals, setOpenModals, action, setOrderSummary }: topUp_propTypes) {
  const [amount, setAmount] = useState<string>("");
  const { membershipApproved } = useGlobal();
  const [number, setNumber] = useState();
  const [offers, setOffers] = useState(["All", "SME", "Direct", "Router", "Unlimited", "Xtra-Data"]);
  ...
  const handleBuyData = (data: any) => {
    setOrderSummary(data);
    setOpenModals((prev: modalTypes) => ({ ...prev, data: false, withdrawal_OTP: true }));
  };
  ...
  return (
    <Modal
      ...
      onClose={() => setOpenModals((prev: modalTypes) => ({ ...prev, data: false }))}
    >
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader className='flex justify-between items-center gap-4'>
              <h1 className='text-2xl text-gray-900 font-bold'>Data</h1>
              <Button onPress={() => setOpenModals((prev: modalTypes) => ({ ...prev, data: false }))} disableRipple className='bg-white outline-none !min-w-11 !w-11 !h-11 rounded-md hover:bg-gray-50 transition-colors flex data-[hover=true]:!bg-gray-50 !opacity-100 items-center justify-center !p-0'>
                <XClose />
              </Button>
            </ModalHeader>
            <ModalBody>
              ...
              <div className='p-2 rounded-2xl bg-neutral-100 overflow-hidden flex-none w-full flex sm:gap-6 gap-3 h-12'>
                <div className='flex p-1 items-center  w-fit flex-none gap-1'>
                  <FilterLines /> <span className='text-sm sm:flex hidden font-medium text-black'>Offers</span>
                </div>
                <div className='w-full flex items-center rounded-r-xl gap-1 overflow-x-scroll scrollbar-none'>
                  {offers.map((offer: string, idx: number) => {
                    return (
                      <Button onPress={() => setActiveOffer(offer)} key={idx} disableRipple className={`${activeOffer === offer ? "bg-primary_fixed" : " !bg-white"} outline-none !min-w-fit !w-fit !h-full rounded-full  transition-colors flex items-center justify-center !px-[16px]`}>
                        <span className={`text-sm font-medium ${activeOffer === offer ? "text-white" : "text-black"}`}>{offer}</span>
                      </Button>
                    );
                  })}
                </div>
            ...
            <ModalFooter>
              <div className='w-full flex relative'>
                <Button onClick={handleSubmit(handleBuyData)} disableRipple className='h-11 gap-x-[6px] !outline-none shadow-main flex flex-none items-center px-[18px] py-3 !min-w-full border-none bg-primary_fixed data-[hover=true]:!bg-brand-700 !opacity-100 transition-colors rounded-lg'>
                  <span className='text-white  text-base font-semibold'>Buy Data</span>
                </Button>
              </div>
            </ModalFooter>
          </>
