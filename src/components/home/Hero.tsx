...
import UtilityPurchases from "../UtilityPurchases";
    ...
    <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} transition={{ type: "spring" }} className='w-full p-4 relative rounded-xl border border-gray-300 bg-white shadow-main-lg'>
      <div className='w-full flex flex-col'>
        ...
        <p className='text-gray-500 text-sm font-normal'>
          You have 13 pending orders that are awaiting payment. click <span className='text-primary_fixed transition-colors cursor-pointer hover:text-brand-700'>here</span> to pay
        </p>
      </div>
...
export default function Hero() {
  ...
  return (
    <motion.section layout className='w-full sm:p-6 p-0 sm:rounded-xl rounded-none flex flex-col flex-none h-fit sm:border sm:gap-y-11 gap-y-0  sm:border-gray-200 border-white sm:bg-white bg-neutral-100'>
      {openPendingRequest && <PendingRequest open={openPendingRequest} setOpen={setOpenPendingRequest} />}
      <div className='w-full flex flex-col sm:mt-0 mt-11 min-h-[200px] overflow-y-scroll scrollbar-none h-full gap-y-6'>
        <Wallet />
        <UtilityPurchases />
