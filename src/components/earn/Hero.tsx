...
export default function Hero({ setOpenModals }: hero_propTypes) {
  ...
  const noticficationBannerAction = () => {
    const element: any = document?.getElementById("earnPageTasks");
    if (element) {
  ...
  useEffect(() => {
    if (membershipApproved !== "false") {
      setOpenBanner(true);
      setNotificationBanner({ open: true, mainContent: "Your membership has been approved", description: "You can now", action: "Generate tasks", actionCallback: () => noticficationBannerAction() });
    }
  }, [membershipApproved]);
    ...
    <motion.section initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ type: "spring" }} layout className='w-full sm:px-6 py-6 px-3 gap-y-6 rounded-xl flex flex-col items-center justify-center flex-none h-fit border border-outline_varient bg-white'>
      ...
      <div className='sm:my-10 py-0 flex flex-col mt-6 h-full w-full justify-center items-center gap-y-6'>
        ...
        {membershipApproved === "pending" || membershipApproved === "approved" ? (
          <Button onPress={() => setOpenModals((prev: modalTypes) => ({ ...prev, becomeMember: true }))} disableRipple className='h-9 gap-x-[6px] !outline-none flex items-center px-[14px] py-2 !min-w-auto bg-brand-50 !border !border-brand-300 data-[hover=true]:!bg-brand-200 !opacity-100 transition-colors rounded-lg'>
            <span className='text-brand-700 text-base font-semibold'>Learn More</span>
          </Button>
        ) : (
          <Button onPress={() => setOpenModals((prev: modalTypes) => ({ ...prev, becomeMember: true }))} disableRipple className='h-9 gap-x-[6px] !outline-none flex items-center px-[14px] py-2 !min-w-auto border-none bg-primary_fixed data-[hover=true]:!bg-brand-700 !opacity-100 transition-colors rounded-lg'>
